...
class Server:
    ...
    def handle_client_enter_username(socket_conn) -> str:
   # Read length (4 bytes)
        length_bytes = socket_conn.recv(4)
        length = struct.unpack('>I', length_bytes)[0]
   
        opcode = socket_conn.recv(1)[0]
   
        username_len = struct.unpack('>H', socket_conn.recv(2))[0]
   
        username = socket_conn.recv(username_len).decode('utf-8')
